...
# uploading orchids images / carico le immagini delle orchidee
...
# initial clustering phase / fase iniziale di clustering
...
# refining phase / fase di refining
...
# manual refining / refining manuale
final_catt <- editLayer(ref_catt, 3, operation = "fill", px_size = 4)
final_cymb <- editLayer(ref_cymb, 2, operation = "fill", px_size = 4) #parameter is 2 for lack of layers / parametro impostato a 2 per mancanza di layer
final_milt <- editLayer(ref_milt, 3, operation = "fill", px_size = 4)
final_rest <- editLayer(ref_rest, 2, operation = "fill", px_size = 4) #parameter is 2 for lack of layers / parametro impostato a 2 per mancanza di layer



## splitByColor() cattleya

    catt_rc <- recolorize2(catt, cutoff = 45, plotting = FALSE) # runs sequentially recolorize() and recluster() / esegue recolorize() e recluster() in modo consecutivo

# layout setting / impostazione della schermata di visualizzazione
    layout(matrix(1:6, nrow = 2, byrow = TRUE))
    par(mar = c(0,0,2,0))

    lay_catt <- splitByColor(catt_rc, plot_method = "overlay") # dividing the color map in single layers / divisione della mappa di colore in singoli layer 
...
### exporting color layers / esportazione dei livelli di colore

    recolorize_to_png(catt_rc, filename = "catt_recolored.png") # creating file .png of recolored image / crea un file .png dell'immagine ricolorata

        for (i in 1:length(lay_catt)) # saving single binary layers in .png / salva i singoli layer binari in formato .png
          {
